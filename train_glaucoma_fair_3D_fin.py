...
from src.utils import model_to_syncbn, Transform3D
...
parser.add_argument('--modality_types', default='rnflt', type=str, help='rnflt|oct_bscans')
...
def set_list_train(in_list, train=True):
    for x in in_list:
        if train:
            x.train()
        else:
...
if __name__ == '__main__':
    ...
    if args.modality_types =='oct_bscans':
        ...
        val_transform = Transform3D(mul='0.5') if args.shape_transform else Transform3D()

    trn_dataset = EyeFair(os.path.join(args.data_dir, 'train'), modality_type=args.modality_types, task=args.task, resolution=args.image_size, attribute_type=args.attribute_type, transform=train_transform)
    tst_dataset = EyeFair(os.path.join(args.data_dir, 'test'), modality_type=args.modality_types, task=args.task, resolution=args.image_size, attribute_type=args.attribute_type, transform=val_transform)
    ...
    if args.modality_types == 'ilm' or args.modality_types == 'rnflt':
        ...
        out_dim = 1
        if True or args.model_type == 'resnet18':
    ...
    ag_norm = ag_norm.to(device)
    final_layer = nn.Linear(in_features=in_feat_to_final, out_features=out_dim, bias=False).to(device)
    model = [model, ag_norm, final_layer]
    parameters_list = []
    for x in model:
    ...
    optimizer = AdamW(parameters_list, lr=args.lr, betas=(0.0, 0.1), weight_decay=args.weight_decay)
    ...
    for epoch in range(start_epoch, args.epochs):
        ...
        if best_auc <= test_auc:
            ...
            'nml_state_dict': model[1].state_dict(),
            'linear_state_dict': model[2].state_dict(),
